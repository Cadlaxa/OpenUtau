...
namespace OpenUtau.Plugin.Builtin {
    [Phonemizer("Malay CV phonemizer", "MS CV", "Cadlaxa", language: "MS")]
    public class MSCVPhonemizer : SyllableBasedPhonemizer {
        protected override string YamlFileName => "ms-vcv-cvvc.yaml";
        protected override string YamlVersion => "1.1";
        protected override byte[] YamlTemplate => MS_VCV_CVVC.data.Resources.template;
        public MSCVPhonemizer() {
            this.vowels = "a,e,i,o,u,ax,x".Split(',');
            this.consonants = Array.Empty<string>();
            this.dictionaryReplacements = "ax=x".Split(';')
        ...
        private readonly Dictionary<string, string> replacements = "ny=n".Split(';')
                .Select(entry => entry.Split('='))
                .Where(parts => parts.Length == 2)
                .Where(parts => parts[0] != parts[1])
                .ToDictionary(parts => parts[0], parts => parts[1]);

        private readonly Dictionary<string, string> missingCphonemes = "ny=n".Split(',')
                .Select(entry => entry.Split('='))
                .Where(parts => parts.Length == 2)
                .Where(parts => parts[0] != parts[1])
                .ToDictionary(parts => parts[0], parts => parts[1]);
        private bool isMissingCPhones = false;
        ...
        protected override List<string> ProcessSyllable(Syllable syllable) {
            ...
            if (HasOto("a ny", syllable.vowelTone) || HasOto("o ny", syllable.vowelTone)) {
                isMissingCPhones = true;
            }
            ...
            if (syllable.IsStartingV) {
                ...
                if (HasOto(rcv, syllable.vowelTone) || HasOto(ValidateAlias(rcv), syllable.vowelTone) || (HasOto(rcv1, syllable.vowelTone) || HasOto(ValidateAlias(rcv1), syllable.vowelTone))) {
                    ...
                    TryAddPhoneme(phonemes, syllable.tone, $"- {cc[0]}", ValidateAlias($"- {cc[0]}"), $"-{cc[0]}", ValidateAlias($"-{cc[0]}"));
                } else {
                    basePhoneme = AliasFormat($"{cc[0]} {v}", "dynMid", syllable.vowelTone, "");
                    TryAddPhoneme(phonemes, syllable.tone, $"- {cc[0]}", ValidateAlias($"- {cc[0]}"), $"-{cc[0]}", ValidateAlias($"-{cc[0]}"));
                }
                ...
                if ((cc.Length - firstC > 1) && CurrentWordCc.Length >= 2) {
                    for (var i = firstC; i < cc.Length; i++) {
                        ...
                        var c_cv = $"{string.Join(" ", cc.Skip(0))}{v}";
                        var rccv = $"- {string.Join("", cc.Skip(0))}{v}";
                        var ucv = $"_{cc.Last()}{v}";
                        if (HasOto(c_cv, syllable.vowelTone) || HasOto(ValidateAlias(c_cv), syllable.vowelTone)) {
                            lastC = 0;
                            basePhoneme = c_cv;
                            break;
                ...
                for (var i = lastC + 1; i >= 0; i--) {
                    var vr = $"{prevV} -";
                    var vcc = cc.Length >= 2 ? $"{prevV} {string.Join("", cc.Take(2))}" : "";
                    var vcc2 = cc.Length >= 2 ? $"{prevV}{string.Join(" ", cc.Take(2))}" : "";
                    var vcc3 = cc.Length >= 3 ? $"{prevV}{string.Join(" ", cc.Take(3))}" : "";
                    var vc = cc.Length > 0 ? $"{prevV} {cc[0]}" : "";
                    var ccTransition = cc.Length >= 2 ? $"{cc[0]}" : ""; 

                    if (i == 0) {
                        if (HasOto(vr, syllable.tone) || HasOto(ValidateAlias(vr), syllable.tone)) {
                            //phonemes.Add(vr);
                        }
                    /*} else if (cc.Length >= 2 && (HasOto(ccTransition, syllable.tone) || HasOto(ValidateAlias(ccTransition), syllable.tone))) {
                        // CV FIX: No VC found, but we have multiple consonants. Use CC alias if it exists.
                        TryAddPhoneme(phonemes, syllable.tone, ccTransition, ValidateAlias(ccTransition));
                        break;
                    } else if (cc.Length > 0) {
                        var ccFallback = AliasFormat($"{cc[1]}", "cc1_mix", syllable.tone, "");
                        TryAddPhoneme(phonemes, syllable.tone, ccFallback, ValidateAlias(ccFallback));
                        break;*/
                    } else {
                        continue;
                    }
            ...
            for (var i = firstC; i < lastC; i++) {
                ...
                if (i + 1 < lastC) {
                    ...
                    if ((HasOto(ccv1, syllable.vowelTone) || HasOto(ValidateAlias(ccv1), syllable.vowelTone) && !PreviousWordCc.Contains(string.Join("", cc.Skip(i + 1))))) {
                        lastC = i;
                        basePhoneme = ccv1;
                        // Use RCCV if it exists
                    } else if ((HasOto(ccv, syllable.vowelTone) || HasOto(ValidateAlias(ccv), syllable.vowelTone)) && CurrentWordCc.Length >= 2 && !PreviousWordCc.Contains(string.Join("", cc.Skip(i + 1)))) {
                        lastC = i;
                        basePhoneme = ccv;
        ...
        protected override List<string> ProcessEnding(Ending ending) {
            ...
            if (ending.IsEndingV) {
                ...
                if (HasOto(vR, ending.tone) || HasOto(ValidateAlias(vR), ending.tone) || HasOto(vR2, ending.tone) || HasOto(ValidateAlias(vR2), ending.tone)) {
                    TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{prevV}", "ending", ending.tone, "", t), ValidateAlias(AliasFormat($"{prevV}", "ending", ending.tone, "", t)));
                }
            } else if (ending.IsEndingVCWithOneConsonant) {
                // Array safety added, though usually true if IsEndingVCWithOneConsonant is true
                var vcr1 = cc.Length > 0 ? $"{v} {cc[0]} {t}" : "";
                var vcr2 = cc.Length > 0 ? $"{v} {cc[0]}{t}" : "";
                var vcr3 = cc.Length > 0 ? $"{v}{cc[0]} {t}" : "";
                var vcr4 = cc.Length > 0 ? $"{v}{cc[0]}{t}" : "";
                var vcSpace = cc.Length > 0 ? $"{v} {cc[0]}" : "";
                string kAlias = $"{v}' {t}";
                ...
                } else if (cc.Length > 0 && cc[0].EndsWith("k") && (HasOto(kAlias, ending.tone) || HasOto(ValidateAlias(kAlias), ending.tone))) {
                    phonemes.Add(HasOto(kAlias, ending.tone) ? kAlias : ValidateAlias(kAlias));
                ...
                } else if (HasOto(vcSpace, ending.tone) || HasOto(ValidateAlias(vcSpace), ending.tone)) {
                    TryAddPhoneme(phonemes, ending.tone, vcSpace, ValidateAlias(vcSpace));
                ...
                for (var i = lastC; i >= 0; i--) {
                    ...
                    // Array safety checks
                    var vcc = cc.Length >= 2 ? $"{v} {string.Join("", cc.Take(2))}{t}" : "";
                    var vcc2 = cc.Length >= 2 ? $"{v}{string.Join(" ", cc.Take(2))}{t}" : "";
                    var vcc6 = cc.Length >= 3 ? $"{v}{string.Join(" ", cc.Take(3))}{t}" : "";
                    var vcc3 = cc.Length >= 2 ? $"{v}{string.Join(" ", cc.Take(2))}" : "";
                    var vcc5 = cc.Length >= 3 ? $"{v}{string.Join(" ", cc.Take(3))}" : "";
                    var vcc4 = cc.Length >= 2 ? $"{v} {string.Join("", cc.Take(2))}" : "";
                    var vc = cc.Length > 0 ? $"{v} {cc[0]}" : "";
                    
                    // New: CC transition check for multiple consonants
                    var ccTransition = cc.Length >= 2 ? $"{cc[0]} {cc[1]}" : "";

                    if (i == 0) {
                        if (HasOto(vr, ending.tone) || HasOto(ValidateAlias(vr), ending.tone)) {
                            TryAddPhoneme(phonemes, ending.tone, vr, ValidateAlias(vr));
                        }
                    } else if (cc.Length >= 2 && (HasOto(vcc, ending.tone) || HasOto(ValidateAlias(vcc), ending.tone)) && lastC == 1) {
                        phonemes.Add(vcc);
                        firstC = 1;
                        break;
                    } else if (cc.Length >= 2 && (HasOto(vcc2, ending.tone) || HasOto(ValidateAlias(vcc2), ending.tone)) && lastC == 1) {
                        phonemes.Add(vcc2);
                        firstC = 1;
                        break;
                    } else if (cc.Length >= 3 && (HasOto(vcc6, ending.tone) || HasOto(ValidateAlias(vcc6), ending.tone)) && lastC == 1) {
                        phonemes.Add(vcc6);
                        firstC = 1;
                        break;
                    } else if (cc.Length >= 2 && (HasOto(vcc3, ending.tone) || HasOto(ValidateAlias(vcc3), ending.tone))) {
                        phonemes.Add(vcc3);
                        ...
                        firstC = 1;
                        break;
                    } else if (cc.Length >= 2 && (HasOto(vcc4, ending.tone) || HasOto(ValidateAlias(vcc4), ending.tone))) {
                        phonemes.Add(vcc4);
                        ...
                        firstC = 1;
                        break;
                    } else if (cc.Length >= 3 && (HasOto(vcc5, ending.tone) || HasOto(ValidateAlias(vcc5), ending.tone))) {
                        phonemes.Add(vcc5);
                        if (vcc5.EndsWith(cc.Last()) && lastC == 1) { // FIXED: Was checking vcc4.EndsWith previously
                            if (affricate.Contains(cc.Last())) {
                                TryAddPhoneme(phonemes, ending.tone, $"{cc.Last()} {t}", ValidateAlias($"{cc.Last()} {t}"), cc.Last(), ValidateAlias(cc.Last()));
                            } else {
                                TryAddPhoneme(phonemes, ending.tone, $"{cc.Last()} {t}", ValidateAlias($"{cc.Last()} {t}"));
                        ...
                        firstC = 1;
                        break;
                    } else if (cc.Length > 0 && (HasOto(vc, ending.tone) || HasOto(ValidateAlias(vc), ending.tone))) {
                        TryAddPhoneme(phonemes, ending.tone, vc, ValidateAlias(vc));
                        break;
                    } else if (cc.Length >= 2 && (HasOto(ccTransition, ending.tone) || HasOto(ValidateAlias(ccTransition), ending.tone))) {
                        // CV FIX: No VC ending, but we have multiple consonants. Use CC alias if it exists.
                        TryAddPhoneme(phonemes, ending.tone, ccTransition, ValidateAlias(ccTransition));
                        firstC = 1;
                        break;
                    } else if (cc.Length > 0) {
                        // CV FIX FALLBACK: Default to the standalone formatted consonant
                        TryAddPhoneme(phonemes, ending.tone, $"{cc[0]}", ValidateAlias($"{cc[0]}"), 
                                                        $"{cc[0]} -", ValidateAlias($"{cc[0]} -"),
                                                        $"{cc[0]}-", ValidateAlias($"{cc[0]}-"));
                        break;
