...
namespace OpenUtau.Plugin.Builtin {
    [Phonemizer("Arpasing Ex Phonemizer", "EN ARPA EX", "Cadlaxa", language: "EN")]
    public class ArpaEX : SyllableBasedPhonemizer {
        ...
        public ArpaEX() {
            this.vowels = new string[] {
                ...
                "oh", "eu", "oe", "yw", "yx", "wx", "ox", "ex", "ea", "ia", "oa", "ua", "ean", "eam", "eang", "3", "0r", "0l"
            };
            ...
            for (var i = firstC; i < lastC; i++) {
                ...
                if (CurrentWordCc.Length >= 2) {
                    ...
                    if (liquid.Contains(cc[i + 1]) || semivowel.Contains(cc[i + 1])
                        || vowels.Contains(ValidateAlias(cc[i + 1], syllable.vowelTone))) {
                        glides(cc1);
                        glides(ValidateAlias(cc1, syllable.tone)); 
                    }
                    // CV
                } else if (CurrentWordCc.Length == 1 && PreviousWordCc.Length == 1) {
                    basePhoneme = (AliasFormat($"{cc.Last()} {v}", "dynMid", syllable.vowelTone, ""));
