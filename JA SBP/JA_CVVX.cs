...
namespace OpenUtau.Plugin.Builtin {
    ...
    public class JA_CVVX : SyllableBasedPhonemizer {
        ...
        public JA_CVVX() {
            this.vowels = "a,i,u,e,o".Split(',');
            this.consonants = "b,by,ch,d,dh,f,g,gy,h,hy,j,k,ky,l,ly,m,my,n,ny,ng,p,py,r,ry,s,sh,t,ts,th,v,w,y,z,zh".Split(',');
        }
        ...
        public override void SetSinger(USinger singer) {
            ...
            if (File.Exists(file)) {
                try {
                    ...
                    // FIX 2: Correct Kana list assignment mapping
                    if (data.wanakana != null) {
                        foreach (var entry in data.wanakana) {
                            ...
                            foreach (var kana in entry.ToList) {
                                if (!KanaToPhonemeDict.ContainsKey(kana)) {
                                    KanaToPhonemeDict.Add(kana, phonemeList);
                                }
                                WanaKanaDictionary[key].Add(kana);
                            }
                        }
                    }
