...
namespace OpenUtau.Plugin.Builtin {
    [Phonemizer("Glockenspiel  German C+V Phonemizer", "GL DE C+V", "Cadlaxa", language: "DE")]
    public class GermanCpVPhonemizer : SyllableBasedPhonemizer {
        private const string LatestVersion = "1.7";
        ...
        protected override string[] GetSymbols(Note note) {
            ...
            for (int k = 1; k < original.Length; k++) {
                if (original[k] == "ex" && vowels.Contains(original[k - 1])) {
                    original[k] = "xe";
                }
                if (!hasPhoneticHint) {
        ...
        protected override IG2p LoadBaseDictionary() {
            ...
            string path = Path.Combine(PluginDir, "de-cPv.yaml");
            if (!File.Exists(path)) {
                Directory.CreateDirectory(PluginDir);
                File.WriteAllBytes(path, German_CpV.data.Resources.template);
            }
            // LOAD DICTIONARY FROM SINGER FOLDER
            if (singer != null || singer.Found || singer.Loaded) {
                string file = Path.Combine(singer.Location, "de-cPv.yaml");
                if (File.Exists(file)) {
                    try {
        ...
        public override void SetSinger(USinger singer) {
            if (this.singer != singer) {
                ...
                if (singer != null && singer.Found && singer.Loaded && !string.IsNullOrEmpty(singer.Location)) {
                    file = Path.Combine(singer.Location, "de-cPv.yaml");
                } else if (!string.IsNullOrEmpty(PluginDir)) {
                    file = Path.Combine(PluginDir, "de-cPv.yaml");
                } else {
                    Log.Error("Singer location and PluginDir are both null or empty. Cannot locate 'de-cPv.yaml'.");
                    return;
                }
                try {
                    ...
                    if (shouldBackupOldFile && File.Exists(file)) {
                        try {
                            ...
                            Log.Warning($"Old de-cPv.yaml has been backed up as: {backupFile}");
                        } catch (Exception e) {
                            Log.Error(e, "Failed to back up old de-cPv.yaml. Proceeding with new template anyway.");
                        }
                    }
                    ...
                    if (shouldWriteTemplate) {
                        try {
                            File.WriteAllBytes(file, German_CpV.data.Resources.template);
                            Log.Information($"'{file}' created or updated to latest version {LatestVersion}");
                        } catch (Exception e) {
                            Log.Error(e, $"Failed to write 'de-cPv.yaml' to {file}");
                        }
                    }
                } catch (Exception ex) {
                    Log.Error(ex, $"Unexpected error while ensuring de-cPv.yaml at {file}");
                }
                ...
                if (File.Exists(file)) {
                    try {
                        ...
                        try {
                            ...
                            Log.Error($"Failed to load vowels and diphthongs from de-cPv.yaml: {ex.Message}");
                        }
                        // Load tails
                        try {
                            ...
                            Log.Error($"Failed to load tails from de-cPv.yaml: {ex.Message}");
                        }
                        ...
                        try {
                            ...
                            Log.Error($"Failed to load replacements from de-cPv.yaml: {ex.Message}");
                        }
                        // Load fallbacks
                        try {
                        ...
                        Log.Error($"Failed to parse de-cPv.yaml: {ex.Message}, Exception Type: {ex.GetType()}");
                    }
                }
