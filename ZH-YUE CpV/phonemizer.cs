...
namespace OpenUtau.Plugin.Builtin {
    [Phonemizer("Cantonese C+V Phonemizer", "ZH-YUE C+V", "Cadlaxa", language: "ZH-YUE")]
    // Custom C+V Phonemizer for OU
    public class CantoneseCpVPhonemizer : SyllableBasedPhonemizer {
        protected override string YamlFileName => "zh-yue-cPv.yaml";
        protected override byte[] YamlTemplate => ZH_YUE_CpV.Data.Resources.template;
        protected override string YamlVersion => "1.2.1";
        public CantoneseCpVPhonemizer() {
        ...
        private readonly Dictionary<string, string> missingVphonemes = "R=-".Split(',')
        ...
        private readonly Dictionary<string, string> missingCphonemes = "nx=n".Split(',')
                .Select(entry => entry.Split('='))
                .Where(parts => parts.Length == 2)
                .Where(parts => parts[0] != parts[1])
                .ToDictionary(parts => parts[0], parts => parts[1]);
        ...
        public override void SetUp(Note[][] groups, UProject project, UTrack track) {
            // 1. Romanize the lyrics first
            RomanizeNotes(groups);
            
            // 2. Allow the SBP to process the now-romanized lyrics
        ...
        protected override List<string> ProcessSyllable(Syllable syllable) {
            ...
            // [V V] or [V C][- C/C][V]/[V]
                ...
                if (!CanMakeAliasExtension(syllable)) {
                    if (HasOto($"{prevV} {v}", syllable.vowelTone) || HasOto(ValidateAlias($"{prevV} {v}"), syllable.vowelTone)) {
                        ...
                        if (diphthongSplits.ContainsKey(prevV)) {
                            ...
                            // FIX: Try to absorb the tail (e.g., 'y') into the next vowel (e.g., 'a') to make 'y a' or 'ya'
                            string cleanTail = tail.TrimEnd('-'); // Remove trailing hyphens if they exist
                            string tailCvSpace = $"{cleanTail} {v}";
                            string tailCvNoSpace = $"{cleanTail}{v}";
                            
                            if (HasOto(tailCvSpace, syllable.vowelTone) || HasOto(ValidateAlias(tailCvSpace), syllable.vowelTone)) {
                                cv = tailCvSpace;
                            } else if (HasOto(tailCvNoSpace, syllable.vowelTone) || HasOto(ValidateAlias(tailCvNoSpace), syllable.vowelTone)) {
                                cv = tailCvNoSpace;
                            } else {
                        ...
                        // FIX: Prioritize the formatted VV alias before falling back to the bare V alias
                        var vvFormat = AliasFormat(v, "vv", syllable.vowelTone, "");
                        
                        if (HasOto(vvFormat, syllable.vowelTone) || HasOto(ValidateAlias(vvFormat), syllable.vowelTone)) {
                            basePhoneme = vvFormat;
                        } else if (HasOto(v, syllable.vowelTone) || HasOto(ValidateAlias(v), syllable.vowelTone)) {
                            basePhoneme = v;
                        } else {
                            basePhoneme = vvFormat;
                ...
                if ((HasOto(cv, syllable.vowelTone) || HasOto(ValidateAlias(cv), syllable.vowelTone)) || (HasOto(crv, syllable.vowelTone) || HasOto(ValidateAlias(crv), syllable.vowelTone))) {
                    basePhoneme = AliasFormat(crv, "dynMid", syllable.vowelTone, "");
                    lastC = 0;
                } else {
                    basePhoneme = AliasFormat(v, "cv", syllable.vowelTone, "");
                }
                ...
                for (var i = firstC; i < cc.Length - 1; i++) {
                    /// CCV
                    if (CurrentWordCc.Length >= 2 && PreviousWordCc.Length == 0) {
                        if ((HasOto(cv, syllable.vowelTone) || HasOto(ValidateAlias(cv), syllable.vowelTone) || HasOto(crv, syllable.vowelTone) || HasOto(ValidateAlias(crv), syllable.vowelTone))) {
                            basePhoneme = AliasFormat($"{cc.Last()} {v}", "dynMid", syllable.vowelTone, "");
                            lastC = i;
                            break;
                        }
                    } else if (CurrentWordCc.Length >= 2) {
                        if ((HasOto(cv, syllable.vowelTone) || HasOto(ValidateAlias(cv), syllable.vowelTone) || HasOto(crv, syllable.vowelTone) || HasOto(ValidateAlias(crv), syllable.vowelTone))) {
                            basePhoneme = AliasFormat($"{cc.Last()} {v}", "dynMid", syllable.vowelTone, "");
                            lastC = 1;
                            break;
                ...
                for (var i = lastC + 1; i >= 0; i--) {
                    ...
                    if (i == 0 && !HasOto(vc, syllable.tone)) {
                        TryAddPhoneme(phonemes, syllable.tone, AliasFormat($"{cc[0]}", "cc", syllable.tone, ""), ValidateAlias(AliasFormat($"{cc[0]}", "cc", syllable.tone, "")));
                        break;
                        /// use vowel ending
            ...
            for (var i = firstC; i < lastC; i++) {
                ...
                if (syllable.CurrentWordCc.Length >= 2) {
                    if ((HasOto(cv, syllable.vowelTone) || HasOto(ValidateAlias(cv), syllable.vowelTone)) || (HasOto(crv, syllable.vowelTone) || HasOto(ValidateAlias(crv), syllable.vowelTone))) {
                        basePhoneme = AliasFormat(crv, "dynMid", syllable.vowelTone, "");
                        lastC = i;
                    } else if ((HasOto($"{v}", syllable.vowelTone) || HasOto($"- {v}", syllable.vowelTone) || HasOto($"-{v}", syllable.vowelTone)) && HasOto(AliasFormat($"{string.Join("", cc.Skip(i + 1))}", "cc", syllable.tone, ""), syllable.vowelTone)) {
                        basePhoneme = AliasFormat(v, "cv", syllable.vowelTone, "");
                        lastC = i;
