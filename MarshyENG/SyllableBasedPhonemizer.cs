                    ...
                    try {
                        ...
                        if (data?.replacements != null) {
                            ...
                            foreach (var rawReplacement in data.replacements) {
                                ...
                                object parsedFrom = rawReplacement.from;
                                object parsedTo = rawReplacement.to;
                                if (rawReplacement.from is System.Collections.IEnumerable fromEnum && !(rawReplacement.from is string)) {
                                    var fromArr = fromEnum.Cast<object>().Select(x => x?.ToString() ?? "").ToArray();
                                    parsedFrom = fromArr.Length == 1 ? fromArr[0] : fromArr;
                                }
                                if (rawReplacement.to is System.Collections.IEnumerable toEnum && !(rawReplacement.to is string)) {
                                    var toArr = toEnum.Cast<object>().Select(x => x?.ToString() ?? "").ToArray();
                                    parsedTo = toArr.Length == 1 ? toArr[0] : toArr;
                                }
