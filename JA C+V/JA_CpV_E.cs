...
namespace OpenUtau.Plugin.Builtin {
    [Phonemizer("Japanese C+V Phonemizer", "JA C+V", "Cadlaxa", language: "JA")]
    // Custom Japanese C+V Phonemizer for OU (Commission)
    public class JA_CpV_E : SyllableBasedPhonemizer {
        ...
        protected override string[] GetSymbols(Note note) {
            ...
            if (original == null) {
                ...
                if (Regex.IsMatch(lyric, @"\p{IsHiragana}+")) {
                    // Try matching longest Hiragana substrings in your dictionary
                    int ja = 0;
                    while (ja < lyric.Length) {
                        string match = null;
                        // Match longer combos first
                        ...
                        if (match == null) {
                            // No match → fallback to Wanakana conversion for that character
                            romaji += WanaKana.ToRomaji(lyric[ja].ToString());
                            ja++;
                        }
                    }
                } else if (Regex.IsMatch(lyric, @"\p{IsKatakana}+")) {
                    // Katakana → use Wanakana
                    romaji = WanaKana.ToRomaji(lyric);
                } else {
        ...
        protected override IG2p LoadBaseDictionary() {
            ...
            string path = Path.Combine(PluginDir, "ja-cPv.yaml");
            if (!File.Exists(path)) {
                Directory.CreateDirectory(PluginDir);
                File.WriteAllBytes(path, Resources.template);
            }
            // LOAD DICTIONARY FROM SINGER FOLDER
            if (singer != null || singer.Found || singer.Loaded) {
                string file = Path.Combine(singer.Location, "ja-cPv.yaml");
                if (File.Exists(file)) {
                    try {
        ...
        public override void SetSinger(USinger singer) {
            if (this.singer != singer) {
                ...
                if (singer != null && singer.Found && singer.Loaded && !string.IsNullOrEmpty(singer.Location)) {
                    file = Path.Combine(singer.Location, "ja-cPv.yaml");
                } else if (!string.IsNullOrEmpty(PluginDir)) {
                    file = Path.Combine(PluginDir, "ja-cPv.yaml");
                } else {
                    Log.Error("Singer location and PluginDir are both null or empty. Cannot locate 'ja-cPv.yaml'.");
                    return;
                }
                try {
                    ...
                    if (shouldBackupOldFile && File.Exists(file)) {
                        try {
                            ...
                            Log.Warning($"Old ja-cPv.yaml has been backed up as: {backupFile}");
                        } catch (Exception e) {
                            Log.Error(e, "Failed to back up old ja-cPv.yaml. Proceeding with new template anyway.");
                        }
                    }
                    ...
                    if (shouldWriteTemplate) {
                        try {
                            File.WriteAllBytes(file, Resources.template);
                            Log.Information($"'{file}' created or updated to latest version {LatestVersion}");
                        } catch (Exception e) {
                            Log.Error(e, $"Failed to write 'ja-cPv.yaml' to {file}");
                        }
                    }
                } catch (Exception ex) {
                    Log.Error(ex, $"Unexpected error while ensuring ja-cPv.yaml at {file}");
                }
                ...
                if (File.Exists(file)) {
                    try {
                        ...
                        try {
                            ...
                            Log.Error($"Failed to load vowels and diphthongs from ja-cPv.yaml: {ex.Message}");
                        }
                        ...
                        try {
                            ...
                            Log.Error($"Failed to load replacements from ja-cPv.yaml: {ex.Message}");
                        }
                        // Load fallbacks
                        try {
                        ...
                        Log.Error($"Failed to parse ja-cPv.yaml: {ex.Message}, Exception Type: {ex.GetType()}");
                    }
                }
